...
import { DEFAULT_SYSYTEM_INSTRUCTION, } from "../utils/constants.js";
...
export async function getChatV2(req, res, next) {
    ...
    try {
        ...
        let conversation;
        if (personalityId) {
            const personality = await Personality.findById(personalityId).select("systemInstruction");
            if (!personality)
                return res.status(404).json({ message: "Personality not found" });
            conversation = await Conversation.findOneAndUpdate({ user: user._id, personality: personalityId }, { $setOnInsert: { user: user._id, personality: personalityId } }, { new: true, upsert: true });
...
export async function getPersonalityMessagesById(req, res, next) {
    ...
    try {
        if (!personalityId ||
            !mongoose.Types.ObjectId.isValid(personalityId)) {
            return res
                .status(400)
...
export async function getPersonalityById(req, res, next) {
    try {
        ...
        const personality = await Personality.findById(id).select("-systemInstruction");
        if (!personality) {
            return res.status(404).json({
                success: false,
                message: "Personality not found",
...
export async function getAllPersonalities(req, res, next) {
    try {
        const { search, featured, id, page = 1, perPage = 100 } = req.query;
        const pageNumber = Math.max(1, parseInt(page, 10) || 1);
        const limit = Math.min(100, Math.max(1, parseInt(perPage, 10) || 100));
        const skip = (pageNumber - 1) * limit;
        const pipeline = [];
        if (id) {
...
export async function getPersonalityByName(req, res, next) {
    try {
